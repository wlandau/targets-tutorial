...
3. You may wish to use `tar_map()` from the `tarchetypes` package to layer static branching on top of dynamic branching. (However, this is not required.)
4. This chapter includes a modified set of functions in `7-challenge/functions.R`. `test_model()` now omits some hyperparameters but includes the sizes of the training and testing datasets. Here is an example collection of model runs. So the results should look something like this.
