...
Now go find that error and fix it! You may have to explore the `6-debugging/functions.R` file, which is where `test_model()` lives.

```{r}
usethis::edit_file("6-debugging/functions.R")
```
...
Some situations may require you to debug this way instead of `error = "save"`. For example, this technique allows you to debug that did not necessarily throw an error. In addition, if some of your target's dependencies cannot be serialized with `qs::qsave()` (e.g. Keras model objects) then the workspace cannot be saved properly with `tar_option_set(error = "save")`.
