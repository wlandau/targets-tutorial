...
`map()` can branch over more than one grouping variable. As an example, let's vary the size of the first layer of the neural network. Make the following changes to `_targets.R`:
...
C. The 3 branches from the earlier `map()` call were still up to date, so they were skipped. `cross()` added 6 additional targets to the pipeline, and those ran for the first time.
