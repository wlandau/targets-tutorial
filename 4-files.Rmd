...
Because we ran the pipeline before, our targets live in the `_targets/` data store. Switch back to `results.Rmd` in the RStudio IDE  and click the `knit` button to generate the output file `results.html`. Then, view `results.html` in the IDE or a web browser.

## R Markdown within a target

For the sake of convenience and reproducibility, we can render this report inside a target in the pipeline. That way, `targets` will automatically regenerate `results.html` whenever its dependencies change. The `tar_render()` function from the `tarchetpyes` package is the correct way to include an R Markdown report with dependencies. It automatically creates a target that reproducibly tracks the R Markdown source file, the rendered output file, and the target dependencies.
