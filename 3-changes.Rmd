...
In this chapter, we will explore the primary motivation behind the `targets` package e. When we modify the code and data of an existing workflow, `targets` automatically detects the changes, and `tar_make()` only reruns the affected targets, skipping all targets that are already up to date. This behavior is the best feature of `targets`, and it saves tons of time when a project is still under development and rapidly changing.

# Instructions

Throughout this chapter, you will test your understanding with quiz questions. Supply your choices to the `answer3_*()` functions when you come to them. Run the code chunk below to set up the quizzes, but try not to peak in advance. It is okay to answer incorrectly at first.
...
Then, open `_targets.R` manually in the RStudio IDE. What do you see?
