...
User-side functions in `targets` rely heavily on the `_targets.R` file. Examples are  `tar_validate()`, `tar_manifest()`, `tar_glimpse()`, and `tar_visnetwork()`. Each of these functions spins up a background process that runs `_targets.R` and then inspects the pipeline in some way.
