...
It is standard practice to make further modifications to `_targets.R` by hand, but for pedagogical simplicity in this chapter, we will use `tar_script()` instead. For now, let's just peek at `_targets.R` whenever we change it.
...
Your turn: add a third model with a different activation function. Use the `act1 = "softmax"` this time.
...
Your turn: write this step as a target below. Note: although commands should usually be concise function calls, but this is not a restrict requirement, as you will see below.
...
Finally, add a new target to retrain the best model with `retrain_run(best_run, churn_recipe)`. This time, we are returning a Keras model object, so we need to write `format = "keras"` in `tar_target()`.
...
Note: you could have manually edited `_targets.R` instead of going through `tar_script()`. In either case, your `_targets.R` file should now look something like this.
